...
5. **Empty space is valid.** 空白和纯文字页可以承载暂停、疑问、对照与结论；不以填充率判定完成度。留白不是省略分析，克制的是装饰而非论证。密集案例不能为了显得高级被拆得失去关系；增加分析密度和形式论证，不以装饰密度替代。
...
统一证据编号、字体角色、标点、图注语法、来源索引与测量单位。允许案例改变主图尺度、文本量、构图和页数；背景与强调色在全局色卡及角色预算内变化，不默认各案例独立配色。共同阅读逻辑不等于共同装饰 motif。
